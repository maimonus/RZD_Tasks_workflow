...
## ✅ Исправление проблемы "Ошибка загрузки ролей"

Если при открытии страницы регистрации видите ошибку **"Не удалось загрузить список ролей"**, это означает что frontend не может подключиться к backend API.

**Проблема:** Frontend и backend запущены на разных портах (3000 и 8000), и нужно явно указать адрес API.

**Решение:** Эта проблема уже исправлена в проекте:
1. Добавлены переменные окружения в `frontend/.env`
2. Улучшена логика определения API URL в `frontend/src/api/api.ts`
3. Добавлена proxy конфигурация в `frontend/vite.config.js`

**Для полной диагностики:**
- Смотрите документ [SETUP_LOCAL.md](SETUP_LOCAL.md) с подробными инструкциями
- Проверьте что backend запущен на `http://localhost:8000` и открывается
- Проверьте что frontend запущен на `http://localhost:3000` и открывается
- Используйте Developer Tools (F12) → Console для тестирования API
